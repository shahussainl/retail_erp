            ...
            <div class="row">
                <div class="">
                    <table id="products-table" class="table table-hover">
                        ...
                        <tbody>
                            <?php if (!empty($PartialsBills)) { ?>
                                <?php
                                $countt = 1;
                                foreach ($PartialsBills as $partialBill) {
                                    $pur_date      = $partialBill['info']->purchase_date;
                                    $pur_date2     = $partialBill['info']->purchase_created_date;
                                    $vendorName    = $partialBill['info']->user_fname;
                                    $billTotal     = $partialBill['info']->purchase_bill_total;
                                    //$paidTotal     = $partialBill->purchase_paid_amount;
                                    $is_ref        = $partialBill['info']->is_ref;
                                    ...
                                    ?>
                                    <tr class="<?php
                                    if ($partialBill['info']->is_stock == 1) {
                                        echo "tbl-green";
                                    } else if ($partialBill['info']->is_stock == 0) {
                                        echo "tbl-warm";
                                    }
                                    ?>">
                                        <td><?= $countt; ?></td>
                                        <td><?= $prefix ?><?= $partialBill['info']->purchase_id; ?></td>
                                         <td><?= $vendorName; ?></td>
                                        <td><?= $purchase_date; ?></td>
                                        <td><?php if($purchase_date != $create_date){
                  ...
                  <ul class="dropdown-menu inv-details">
                      <table class="table table-condensed table-bordered table-sm">
                          ...
                          <?php $count = 1;$g_total = 0;  foreach($partialBill['items'] as $bill_items){ ?>
                          
                          <tr>
                              <td><?= $count; ?></td>
                              <td><?= $bill_items->prd_title; ?></td>
                              <td><?= $bill_items->puritem_qty; ?></td>
                              <td><?php if(!empty($currency_symbol->symbol)){ echo $currency_symbol->symbol; } echo number_format($bill_items->puritem_price,2); ?></td>
                              <td><?php $s_total = $bill_items->puritem_price * $bill_items->puritem_qty; if(!empty($currency_symbol->symbol)){ echo $currency_symbol->symbol; }  echo number_format($s_total,2);$g_total = $g_total + $s_total;  ?></td>
                          </tr>
                                <?php $count++; } ?>
                          <tr>
                              <td colspan="4" class="text-right"><b>Grand Total</b></td>
                              <td><b><?php if(!empty($currency_symbol->symbol)){ echo $currency_symbol->symbol; } echo number_format($g_total,2); ?></b></td>
                          </tr>
                      </table>
                  </ul>
                </div>
                                        </td>
                                        <td><?php if(!empty($currency_symbol->symbol)){ echo $currency_symbol->symbol; } ?>
                                            <?php echo number_format($billTotal,2);                         ?>
                                        </td>
                                        <td>
                                            <?php 
                                            $t_payment = 0;
                                            foreach($partialBill['payments'] as $sin_payment){
                                            $t_payment  = $t_payment + $sin_payment->purpayment_amount;   
                                            }
                                            if(!empty($currency_symbol->symbol)){ echo $currency_symbol->symbol; } 
                                        ...
                                        <td><?php
                                            ...
                                            <?php } else if ($partialBill['info']->is_stock == 0) { ?>
...
<!--                                       <?php if(!empty($currency_symbol->symbol)){ echo $currency_symbol->symbol; } ?></span>
                                            <?= $paidTotal;                         ?></td>-->
                                            <td class="text-right">
                                            <a href="<?= base_url('Purchase/SinglePurchaseView/' . $partialBill['info']->purchase_id); ?>" class="btn btn-sm btn-default btn-flat table-btn"><i class="fa fa-eye"></i></a>
                                            <a class="btn btn-sm btn-info btn-flat table-btn"  href="<?= base_url('Purchase/SinglePurchasePrint/' . $partialBill['info']->purchase_id); ?>"><i class="fa fa-print"></i></a>
                                            </td>
                                    </tr>
                                <?php $countt++; } ?>
                            <?php } ?>

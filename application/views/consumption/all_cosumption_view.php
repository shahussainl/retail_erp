...
<div class="content-wrapper">
    ...
    <form method="post" class="allSalesInvoice" action="<?= base_url('Sales/changeCheckSalesStatus') ?>">
                        ...
                        <table id="products-table" class="table table-hover table-condensed">
                            <thead>
                                <tr>
                                    ...
                                    <th>Consumpted Product</th>
                                    <th>Creation Date</th>
                                    
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (!empty($all_sale_vouchers)) { ?>
                                    <?php $x = 1;
                                    foreach ($all_sale_vouchers as $single_sale) {
                                        ?>
                                        <tr class="<?php
                                        if ($single_sale['info']->is_invoice == 0 && $single_sale['info']->sales_status == 0) {
                                            echo "tbl-warm";
                                        } elseif (isset($single_sale['info']->cancelation_date)) {
                                            echo "tbl-red";
                                        } else {
                                            echo "tbl-green";
                                        }
                                        ?>">
                                            <td>
        <?php //echo '<pre>';print_r($single_sale['info']);  ?> 
                                            </td>
                                            <td>
        <?= $single_sale['info']->sales_number; ?>
                                            </td>
                                            <td>
        <?= $single_sale['info']->user_fname; ?> <?= $single_sale['info']->user_lname; ?>
                                            </td>
                                            <td>
        <?= date('m/d/Y', strtotime($single_sale['info']->sales_date)); ?>
                                            </td>
                                            <td>
                                                <?php
                                                if ($single_sale['info']->sales_date != $single_sale['info']->sales_created_date) {
                                                ...
                                                <div class="input-group-btn">
                                                    ...
                                                    <ul class="dropdown-menu inv-details">
                                                        <table class="table table-condensed table-bordered table-sm">
                                                            ...
                                                            $count = 1;
                                                            $g_total = 0;
                                                            foreach ($single_sale['items'] as $bill_items) {
                                                                ?>
                                                                <tr>
                                                                    <td><?= $count; ?></td>
                                                                    <td><?= $bill_items->prd_title; ?></td>
                                                                    <td><?= $bill_items->salitem_qty; ?></td>
                                                                    <td><?= number_format($bill_items->salitem_price, 2); ?></td>
                                                                    <td><?php
                                                                $s_total = $bill_items->salitem_price * $bill_items->salitem_qty;
        ...
        ?>
                                ...
                                ?>
                                <?= number_format($single_sale['info']->sales_bill_total, 2); ?></td>
                                            <td>
                                                <?php
                                if (!empty($currency_symbol->symbol)) {
                                    echo $currency_symbol->symbol;
                                ...
                                ?>
                                                ...
                                                <?php
                                                $rec = 0;
                                                foreach ($single_sale['receipt'] as $receipts) {
                                                    $rec = $rec + $receipts->salpayment_amount;
                                                }
                                ...
                                ?> <?php $dif = $single_sale['info']->sales_bill_total - $rec;
                                        echo number_format($dif, 2);
                                                ?></td>
                                            <td>
                                                <?php
                                                $b_balance = $single_sale['info']->sales_bill_total - $rec;
                                                if ($single_sale['info']->post_status == 0 && $single_sale['info']->is_invoice == 0 && $single_sale['info']->sales_status == 0) {
                                                    echo "<span class='badge bg-yellow'>QUOTATION</span>";
                                                } elseif ($single_sale['info']->is_invoice != 0 && $rec == 0) {
                                                    echo "<span class='badge bg-green'>OPEN INVOICE</span>";
                                                } elseif (isset($single_sale['info']->cancelation_date)) {
                                                    echo "<span class='badge bg-red'>CANCELLED</span>";
                                                } elseif ($rec != 0 && $b_balance > 0) {
                                                    echo "<span class='badge bg-green'>PARTIAL PAID</span>";
                                                } else {
                                            ...
                                            <td class="text-right">
                                                <a class="btn btn-sm btn-default btn-flat table-btn" href="<?= base_url('Sales/singleSaleVoucher/' . $single_sale['info']->sales_id); ?>"><i class="fa fa-eye"></i></a>
                                                <a class="btn btn-sm btn-info btn-flat table-btn" href="<?= base_url('Sales/print_bill/' . $single_sale['info']->sales_id) ?>"><i class="fa fa-print"></i></a>
                                                   <?php if ($single_sale['info']->is_invoice == 0 && $single_sale['info']->sales_status == 0) { ?>
                                                    <a class="btn btn-sm btn-success btn-flat table-btn" href="<?= base_url('Sales/makeInvoice/' . $single_sale['info']->sales_id) ?>" onclick="return confirm('Do you want to create invoice?');"><i class="fa fa-dollar"></i></a>
                                                    <a class="btn btn-sm btn-danger btn-flat table-btn" onclick="$('.cancelSingleSaleBillId').val('<?= $single_sale['info']->sales_id ?>');$('#cancel-single-sale').modal('show');"><i class="fa fa-times"></i></a>
                                        <?php } ?>
                                        <?php if ($single_sale['info']->is_invoice == 1 && $single_sale['info']->post_status == 0) { ?>
                                                    <a class="btn btn-sm btn-success btn-flat table-btn" onclick="$('.post_invoice_sales_id').val('<?= $single_sale['info']->sales_id ?>');
                                                                        $('#paid_amt').attr('max',<?= $single_sale['info']->sales_bill_total ?>);
                                                                        $('#post-payment-voucher-danger').modal('show');"><i class="fa fa-dollar"></i></a>
        <?php } ?> 
                                            </td>
                                        </tr>
        <?php $x++;
...
<?php } ?>
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="5" class="text-right"><label>TOTALS</label></td>
        ...
        <div class="modal fade" id="cancel-all-sale">
...
<?php for ($x = 1; $x <= 3; $x++) { ?>
                                    ...
                                    <tr>
                                              ...
                                              <div class="form-group ">
                                                  <select class="form-control select2" name="item[<?= $x ?>][prd_id]" onchange="getProductDetail(this.value, 'prd_id', this);" style="width: 100%;">
...
<?php } ?>
                        ...
                        <button type='button' onclick="if ($('#sub_total').val() != '0.00') {
                                    createInvoice(this);
                                }" class="btn btn-warning btn-flat"><i class="fa fa-save"></i> CREATE INVOICE</button>
                        <button type='button' onclick="if ($('#sub_total').val() != '0.00') {
...
<!--model code for posting voucher start-->
<div class="modal modal-default fade" id="post-payment-voucher-danger">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            <span class="input-group-addon"><?php if(!empty($currency_symbol)) { echo $currency_symbol->symbol; } ?></span>
...
<?php } ?>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-flat btn-default" data-dismiss="modal">CLOSE POPUP</button>

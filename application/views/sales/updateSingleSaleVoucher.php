...
if (!empty($voucher_data)) {
    ...
    if (!empty($voucher_data['sales'])) {
        ...
        $user_name = $voucher_data['sales']->user_fname;
        $user_contact = $voucher_data['sales']->user_contact;
        $user_address = $voucher_data['sales']->user_address;
                                ...
                                <thead>
                                    <tr>
                                        <th colspan="2">                 <div class="input-group form-group" style="width:100%;">
                                                ...
                                                <input type="text" class="form-control" name="vendor_id" required value="<?= $customer_id ?>" <?php if ($is_ref == 1 || $sales_status == 1) { ?> disabled <?php } ?> <?php if ($post_status > 0 || $is_invoice > 0) { ?> readonly <?php } ?> onblur="getUserDetail(this.value);">
                                    ...
                                    <tr>
                                        <th colspan="2">                 <div class="form-group input-group" style="width:100%;">
                                                <span class="input-group-addon"><i class="fa fa-user"></i> <b>Full Name</b></span>
                                                <input type="text" class="form-control" id="customer_full_name" name="customer_full_name" required value="<?= $user_name; ?>" readonly>

                                            </div></th>
                                        <th colspan="2">
                                            <div class="form-group input-group" style="width:100%;">
                                                <span class="input-group-addon"><i class="fa fa-building"></i> <b>Address</b></span>
                                                <input type="text" class="form-control" id="customer_address" name="customer_address" value="<?= $user_address; ?>" readonly>

                                            </div>
                                        </th>
                                        <th>
                                            <div class="form-group input-group">
                                                <span class="input-group-addon"><i class="fa fa-phone"></i> <b>Contact</b></span>
                                                <input type="text" class="form-control" id="customer_contact" name="customer_contact" value="<?= $user_contact; ; ?>"  readonly >
                                                        ...
                                                        <?php } else { ?>
                                                            <?php if ($sales_status == 0) { ?>
                                                                <?php if ($is_invoice == 0) { ?>
                                                                    ...
                                                                    <a href="<?= base_url('Sales/changeStatusToInvoice/' . $voucher_no.'/'.$user_name); ?>"  class="btn btn-flat btn-warning" onclick="return confirm('Do you want to create invoice?')">CREATE INVOICE</a>
                                                                <?php } ?>
                                                                <button type="button" class="btn btn-flat btn-info" onclick="openModal();"> POST INVOICE</button>
                                                                <?php if ($is_invoice == 0) { ?>
...
//        var cust = $('#customer_id').val();
        ...
        $.ajax({
            ...
            url: '<?= base_url('Sales/getUserDetail'); ?>',
            success: function (data)
            {
                if (data == null) {
                    ...
                    $('#customer_full_name').attr('readonly', false);
                    $('#customer_address').attr('readonly', false);
                    $('#customer_contact').attr('readonly', false);
                } else {
                    $('#customer_full_name').val(data.user_fname);
                    $('#customer_address').val(data.user_address);
                    $('#customer_contact').val(data.user_contact);

                    $('#customer_full_name').attr('readonly', true);
                    $('#customer_address').attr('readonly', true);
                    $('#customer_contact').attr('readonly', true);
                }
            }
        });
    }
...
//    alert(sub_total);
    ...
    $(document).on('blur', '#paid_amt', function () {
        $('#paid_amt').val(parseFloat($(this).val()).toFixed(2));
    });

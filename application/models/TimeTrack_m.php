<?php
	...
	class TimeTrack_m extends CI_Model
	{
...
// get all timer activities of single user

    public function get_all_times_logs($user_id)
    {
    	 $res =  $this->db->select('*')
                        ->from('users as usr')
                        ->join('timer as tmr','usr.user_id=tmr.user_id')
                        ->join('timer_record as t_rec','tmr.timer_id=t_rec.timer_id')
                        ->where('usr.user_id',$user_id)
                        ->limit(1)
                        ->order_by('t_rec.timer_record_id','desc')
                        ->get()->row_array();
    ...
    public function getTotalDays($user_id)
    {
        $record = $this->db->where('user_id',$user_id)->get('timer')->result();
        $result = [];
        
        foreach($record as $r)
        {
            $result[$r->timer_id]['date'] = $r->timer_date;
            $result[$r->timer_id]['start'] = $this->db->where('timer_id',$r->timer_id)->where('timer_record_status','start')->get('timer_record')->row();
            $result[$r->timer_id]['close'] = $this->db->where('timer_id',$r->timer_id)->where('timer_record_status','close')->get('timer_record')->row();
            $result[$r->timer_id]['detail'] = $this->db->where('timer_id',$r->timer_id)->get('timer_record')->result();
        }
    ...
    {
        return $this->db->join('timer_record','timer_record.timer_id=timer.timer_id')->where('user_id',$user_id)->where('timer_current_day',date('Y-m-d'))->get('timer')->result();
    }
}
